...
export default function ReadingPage() {
  ...
  useEffect(() => { setFavs(getFavorites()); setScale(getFontScale()); const p = getProgress(); setCount(Math.min(p[item.id] ?? 0, item.repetitions)); }, [item]);
  ...
  function changeFont(delta: number) { const next = Math.min(1.25, Math.max(0.9, Number((scale + delta).toFixed(2)))); setScale(next); setFontScale(next); }
